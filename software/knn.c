...
int main() {
  ...
  for (int i=0; i<N; i++) {
    ...
    if (b == 0) max = data[i].x;
    if(b < 100 && data[i].x > max) max = data[i].x;
    b = b + 1;
    if (b < 100) uart_printf("Gen number %d: %d\n", b, data[i].x);
    data[i].y = (short) cmwc_rand();
    if(b < 100 && data[i].y > max) max = data[i].y;
    b = b + 1;
    if (b < 100) uart_printf("Gen number %d: %d\n", b, data[i].y);

    //init label
    data[i].label = (unsigned char) (cmwc_rand()%C);
    if(b < 100 && (int)data[i].y > max) max = (int)data[i].y;
    b = b + 1;
    if (b < 100) uart_printf("Gen number %d: %d\n", b, (int)data[i].label);
  }
